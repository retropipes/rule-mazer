...
class LayeredTower implements Cloneable {
    ...
    public MazeObject getCell(final int fR, final int fC, final int fF, final int extra) {
	int row = fR;
	int col = fC;
	int floor = fF;
	...
	boolean result = true;
	final int zLoc = Main.getApplication().getGameManager().getPlayerManager().getPlayerLocationZ();
    ...
    public void setCell(final MazeObject mo, final int fR, final int fC, final int fF, final int extra) {
	int row = fR;
	int col = fC;
	int floor = fF;
	if (this.verticalWraparoundEnabled) {
	...
	int N = 1;
	for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
	    MazeObject[] objectsWithPrerequisites = objects.getAllWithNthPrerequisite(e, N);
	    while (objectsWithPrerequisites != null) {
		r = new RandomRange(0, objectsWithPrerequisites.length - 1);
		for (x = 0; x < columns; x++) {
		    for (y = 0; y < rows; y++) {
			final MazeObject placeObj = objectsWithPrerequisites[r.generate()];
			final boolean okay = placeObj.shouldGenerateObject(maze, x, y, z, w, e);
			if (okay) {
			    this.setCell(objects.getNewInstanceByName(placeObj.getName()), y, x, z, e);
			    placeObj.editorGenerateHook(y, x, z);
		...
		N++;
		objectsWithPrerequisites = objects.getAllWithNthPrerequisite(e, N);
	    }
	}
	// Pass N + 1
	...
	if (withoutRuleSets != null) {
	    ...
	    int N = 1;
	    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
		MazeObject[] objectsWithPrerequisites = MazeObjectList.getAllWithNthPrerequisiteSubset(withoutRuleSets,
			e, N);
		while (objectsWithPrerequisites != null) {
		    r = new RandomRange(0, objectsWithPrerequisites.length - 1);
		    for (x = 0; x < columns; x++) {
		    ...
		    N++;
		    objectsWithPrerequisites = MazeObjectList.getAllWithNthPrerequisiteSubset(withoutRuleSets, e, N);
		}
	    }
	    // Pass N + 1
	    for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
	...
	if (withRuleSets != null) {
	    ...
	    int N = 1;
	    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
		MazeObject[] objectsWithPrerequisites = MazeObjectList.getAllWithNthPrerequisiteSubset(withRuleSets, e,
			N);
		while (objectsWithPrerequisites != null) {
		    r = new RandomRange(0, objectsWithPrerequisites.length - 1);
		    for (x = 0; x < columns; x++) {
		    ...
		    N++;
		    objectsWithPrerequisites = MazeObjectList.getAllWithNthPrerequisiteSubset(withRuleSets, e, N);
		}
	    }
	    // Pass 2N + 4
	    for (int layer = 0; layer < MazeConstants.LAYER_COUNT; layer++) {
