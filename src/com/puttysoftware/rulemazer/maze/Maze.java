...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "RuleMazer" + File.separator + randomID
		+ ".maze";
	...
	return System.getProperty("java.io.tmpdir") + File.separator + "RuleMazer";
    }
	...
	final double percentMultiplier = percent / (double) GenericCharacter.FULL_HEAL_PERCENTAGE;
	...
	if (this.levelCount > 1) {
		...
		final boolean deleted = this.getLevelFile(this.activeLevel).delete();
		if (!deleted) {
		    return false;
		}
