...
public class Main {
    ...
    private static final Ghosted debug = new Ghosted(Main.PROGRAM_NAME, Main.ERROR_MESSAGE, Main.ERROR_TITLE);
	...
	try {
	    ...
	    platform.hookAbout(Main.application.getAboutDialog(),
		    Main.application.getAboutDialog().getClass().getDeclaredMethod("showAboutDialog"));
	    String s;
	    if (args.length == 0) {
		s = null;
	    } else {
	    ...
	    platform.hookFileOpen(Main.application.getMazeManager(),
		    Main.application.getMazeManager().getClass().getDeclaredMethod("loadFromOSHandler", String.class),
		    s);
	    ...
	    platform.hookQuit(Main.application.getGUIManager(),
		    Main.application.getGUIManager().getClass().getDeclaredMethod("quitHandler"));
