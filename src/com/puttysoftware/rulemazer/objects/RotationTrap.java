...
public class RotationTrap extends GenericTrap implements Cloneable {
	...
	Main.getApplication().showMessage(this.getName() + " (Radius " + this.radius + ", Direction " + dir + ")");
    }
	...
	if (this.direction) {
	    Main.getApplication().getGameManager().doClockwiseRotate(this.radius);
	} else {
	    Main.getApplication().getGameManager().doCounterclockwiseRotate(this.radius);
	}
