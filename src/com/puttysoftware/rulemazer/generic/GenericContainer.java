...
public abstract class GenericContainer extends GenericLock {
	...
	if (!app.getGameManager().isEffectActive(MazeEffectConstants.EFFECT_GHOSTLY)
	    ...
	    Main.getApplication().getGameManager().addToScore(GenericLock.SCORE_UNLOCK);
	} else {
	...
	if (!this.inside.getName().equals("Empty")) {
	    Main.getApplication().showMessage(this.getName() + ": Contains " + this.inside.getName());
	} else {
	    Main.getApplication().showMessage(this.getName() + ": Contains Nothing");
	}
    }
