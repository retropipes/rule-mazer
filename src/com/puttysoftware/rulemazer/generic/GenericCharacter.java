...
public abstract class GenericCharacter extends MazeObject {
	...
	if (arrowType == ArrowTypeConstants.ARROW_TYPE_PLAIN) {
	    Main.getApplication().getMazeManager().getMaze().doDamage(GenericCharacter.SHOT_SELF_NORMAL_DAMAGE);
	} else {
	    Main.getApplication().getMazeManager().getMaze().doDamage(GenericCharacter.SHOT_SELF_SPECIAL_DAMAGE);
	}
