...
public class GameManager implements MazeEffectConstants {
	...
	this.lastUsedBowIndex = Main.getApplication().getObjects().getAllBowNames().length - 1;
	this.knm = false;
	...
	final GenericCharacter player = (GenericCharacter) Main.getApplication().getMazeManager().getMaze().getCell(px,
		py, pz, MazeConstants.LAYER_OBJECT);
	...
	final MazeObject acted = Main.getApplication().getMazeManager().getMazeObject(x, y, z,
		MazeConstants.LAYER_OBJECT);
	...
	do {
	    try {
		try {
		    o = m.getCell(px + x, py + y, pz, MazeConstants.LAYER_OBJECT);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    o = new Empty();
		}
		try {
		...
		try {
		    nextBelow = m.getCell(px + x, py + y, pz, MazeConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextBelow = new Empty();
		}
		try {
		    nextAbove = m.getCell(px + x, py + y, pz, MazeConstants.LAYER_OBJECT);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextAbove = new Wall();
		}
		try {
		    previousBelow = m.getCell(px - x, py - y, pz, MazeConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    previousBelow = new Empty();
		}
		try {
		    nextNextBelow = m.getCell(px + 2 * x, py + 2 * y, pz, MazeConstants.LAYER_GROUND);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		    nextNextBelow = new Empty();
		}
		try {
		    nextNextAbove = m.getCell(px + 2 * x, py + 2 * y, pz, MazeConstants.LAYER_OBJECT);
		} catch (final ArrayIndexOutOfBoundsException ae) {
		...
		try {
			...
			acted = new Empty();
			try {
			    acted = m.getCell(px - x, py - y, pz, MazeConstants.LAYER_OBJECT);
			} catch (final ArrayIndexOutOfBoundsException ae) {
			    // Do nothing
			}
	    ...
	    if (redrawsSuspended && !this.checkLoopCondition(proceed, x, y, groundInto, below, nextBelow, nextAbove)) {
		// Redraw post-suspend
		this.redrawMaze();
		redrawsSuspended = false;
	    }
	} while (this.checkLoopCondition(proceed, x, y, groundInto, below, nextBelow, nextAbove));
	this.getStatGUI().updateStats();
	this.checkGameOver();
		...
		for (y = this.vwMgr.getViewingWindowLocationY(); y <= this.vwMgr
			...
			if (this.trueSightFlag) {
			    this.drawGrid[xFix][yFix].setIcon(GraphicsManager
				    .getImage(new EmptyVoid().editorRenderHook(y, x, this.plMgr.getPlayerLocationZ())));
			} else {
			    this.drawGrid[xFix][yFix].setIcon(GraphicsManager
    ...
    public void decay() {
	if (this.actingRemotely) {
	    Main.getApplication().getMazeManager().getMaze().setCell(new Empty(), this.remoteCoords[0],
	...
	if (this.actingRemotely) {
	    Main.getApplication().getMazeManager().getMaze().setCell(decay, this.remoteCoords[0], this.remoteCoords[1],
	...
	if (this.actingRemotely) {
	    Main.getApplication().getMazeManager().getMaze().setCell(this.delayedDecayObject, this.remoteCoords[0],
		    this.remoteCoords[1], this.remoteCoords[2], MazeConstants.LAYER_OBJECT);
	} else {
	...
	final String result = CommonDialogs.showInputDialog("Use which item?", "RuleMazer", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			Main.getApplication().showMessage("That item has no more uses left.");
			this.setUsingAnItem(false);
		    } else {
			Main.getApplication().showMessage("Click to set target");
		    }
		    return;
		}
	    }
	...
	final String result = CommonDialogs.showInputDialog("Switch to which bow?", "RuleMazer", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
		    ...
		    if (this.objectInv.getUses(this.activeBow) == 0) {
			Main.getApplication().showMessage("That bow is out of arrows!");
		    } else {
			Main.getApplication().showMessage(this.activeBow.getName() + " activated.");
		    }
		    return;
		}
	    }
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
		...
		if (!target.isDestroyable() && name.equals(new WallMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    Main.getApplication().showMessage("Can't create a wall there");
		}
		if (!target.isDestroyable() && name.equals(new FinishMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    Main.getApplication().showMessage("Can't create a finish there");
		}
		...
		if (!target.getName().equals(new Empty().getName()) && !target.getName().equals(new DarkGem().getName())
		    ...
		    this.setUsingAnItem(false);
		    Main.getApplication().showMessage("Aim at either an empty space or a Dark Gem");
		}
		if (!target.getName().equals(new Empty().getName())
			&& !target.getName().equals(new LightGem().getName())
			&& name.equals(new DarkWand().getName())) {
		    this.setUsingAnItem(false);
		    Main.getApplication().showMessage("Aim at either an empty space or a Light Gem");
		}
	    } catch (final ArrayIndexOutOfBoundsException ae) {
		this.setUsingAnItem(false);
