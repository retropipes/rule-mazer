...
public class MazeEditor {
	...
	this.treasurePicker.updatePickerLayout(GraphicsManager.MAX_WINDOW_SIZE);
	...
	if (w != 0) {
	    // Level Change
	    Main.getApplication().getMazeManager().getMaze().switchLevelOffset(w);
	    this.fixLimits();
	    this.setUpGUI();
	}
	this.checkMenus();
	...
	try {
		...
		if (mo2 == null) {
		    Main.getApplication().showMessage("This object has no properties");
		} else {
		    this.checkTwoWayTeleportPair(this.elMgr.getEditorLocationZ());
		    this.updateUndoHistory(this.savedMazeObject, gridX, gridY, this.elMgr.getEditorLocationZ(),
			    this.elMgr.getEditorLocationW(), this.elMgr.getEditorLocationE());
	...
	if (choice != null) {
		...
		if (this.conditionalEditFlag == MazeEditor.CEF_CONDITION) {
		    ...
		    final String resp = CommonDialogs.showTextInputDialogWithDefault(
			    "Condition Trigger Value (Number of Sun Stones needed)", "Editor", def);
		    int respVal = -1;
		    try {
	...
	case TELEPORT_TYPE_TWOWAY:
	    Main.getApplication().showMessage("Click to set teleport destination");
	    break;
	case TELEPORT_TYPE_MOVING_FINISH:
	    Main.getApplication().showMessage("Click to set next moving finish");
	    break;
	case TELEPORT_TYPE_FIRST_MOVING_FINISH:
	    Main.getApplication().showMessage("Click to set first moving finish");
	    break;
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with default data")) {
	    Main.getApplication().getMazeManager().getMaze().fillFloorDefault(this.elMgr.getEditorLocationZ());
	...
	if (this.confirmNonUndoable("overwrite the active level with random data")) {
		...
		Main.getApplication().getMazeManager().getMaze().fillLevelRandomlyCustom();
	    } else {
		Main.getApplication().getMazeManager().getMaze().fillLevelRandomly();
	    }
	...
	if (this.confirmNonUndoable("overwrite the active floor within the active level with random data")) {
		...
		Main.getApplication().getMazeManager().getMaze().fillFloorRandomly(this.elMgr.getEditorLocationZ());
	    }
	...
	if (!this.goToDestMode) {
	    ...
	    Main.getApplication().showMessage("Click a teleport to go to its destination");
	}
    }
	...
	if (this.goToDestMode) {
	    ...
	    final MazeObject there = Main.getApplication().getMazeManager().getMazeObject(locX, locY, locZ,
		    MazeConstants.LAYER_OBJECT);
		...
		Main.getApplication().showMessage("This object does not have a destination.");
	    }
	}
    }
